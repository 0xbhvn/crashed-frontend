...
import { useRiskRewardProfiles, type RiskRewardData } from './useRiskRewardProfiles';
import { useAnalytics } from '@/context/analytics-context';

interface UseRealTimeRiskRewardProfilesOptions {
  targets?: number[];
  limit?: number;
  enabled?: boolean;
}

export function useRealTimeRiskRewardProfiles({
  targets = [1.5, 2, 3, 5, 10, 20],
  limit = 1000,
  enabled = true,
}: UseRealTimeRiskRewardProfilesOptions = {}) {
  // Keep local copy of data to prevent loading states
  const [localData, setLocalData] = useState<RiskRewardData | null>(null);
