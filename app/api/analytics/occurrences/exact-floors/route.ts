...
export async function POST(request: NextRequest) {
	try {
		...
		const backendUrl = getApiUrl('analytics/occurrences/exact-floors');
		...
		if (!backendResponse.ok) {
			...
			console.error(
				'⚠️ Backend response not OK (exact floors):',
				errorText
			);
			throw new Error(
		...
		if (!data.data || typeof data.data !== 'object') {
			console.warn(
				'⚠️ Unexpected data structure from backend (exact floors):',
				data
			);
		...
		console.error('Error proxying API request (exact floors):', error);
