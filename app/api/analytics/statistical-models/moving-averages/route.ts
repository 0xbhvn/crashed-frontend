...
/**
 * GET handler for the moving averages statistical model API endpoint
...
export async function GET(request: NextRequest) {
	try {
		...
		const windows = searchParams.get('windows') || '5,10,20';
		...
		const backendUrl = getApiUrl('analytics/statistical-models/moving-averages');
		...
		if (!backendResponse.ok) {
			...
			console.error('Moving averages API request failed:', errorText);
			throw new Error(
		...
		console.error('Error proxying moving averages API request:', error);

		// Return an appropriate error response
		return NextResponse.json(
			{
				status: 'error',
				message: 'Failed to fetch moving averages data from the API',
